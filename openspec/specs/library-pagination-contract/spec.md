## Purpose
Define a single, stable pagination model shared by all paginated library API endpoints so SDKs implement pagination behavior from one source rather than inventing per-endpoint structures.

...
- [x] 1.3 Update `order-products-listing` to reference `library-pagination-contract` as a formal pagination dependency
...
- [x] 3.1 Refine the `prepare` endpoint response schema in `openapi.yaml` and update `library-resource-contract` to reflect the stable shape
- [x] 3.2 Validate that downstream SDK and app proposals derive library resource types from the API-defined contracts rather than restating them independently
